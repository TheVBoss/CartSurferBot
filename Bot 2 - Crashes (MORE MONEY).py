...
CrashOn10 = False
CrashOn11 = False
TricksterCheck = False
start_time = None
SecondWaveCrash = False
TricksterCrash = False
CrashCheck = False
...
def BackFlip(sct):
    ...
    while time.time() - start_time < 0.5:
        ...
        if signFound(img) and not CrashCheck:
            count = count + 1
            subtract = -0.01*(count*count) + 0.19*(count) - 0.18
            if CheckSign(img):
                direction = True
            else:
                direction = False
...
def IntentionalCrash(sct):
    ...
    global CrashOn10
    global CrashOn11
    global TricksterCheck
    global TricksterCrash
    global SecondWaveCrash
    global CrashCheck
    Turns += 1
    print("Turn number: " + str(Turns))
    now = time.time()
    if last_turn_time is not None:
        delta = now - last_turn_time
    last_turn_time = now
    if Turns == 6 or (SecondWaveCrash and (CrashOn9 or CrashOn10 or CrashOn11)) or ((CrashOn9 and Turns == 11) or (CrashOn10 and Turns == 12 and not TricksterCrash) or (CrashOn11 and Turns == 13) or (TricksterCrash and Turns == 11)):
        SecondWaveCrash = False
        CrashCheck = True
    ...
    if Turns == 8 and delta > 3.5:
        CrashOn9 = True
        SecondWaveCrash = True
    if Turns == 9 and delta > 3.5 and not CrashOn9:
        CrashOn10 = True  
        SecondWaveCrash = True
        TricksterCheck = True
    if Turns == 10 and delta > 3.5 and not CrashOn9 and not CrashOn10:
        SecondWaveCrash = True
        CrashOn11 = True
    return True
                        ...
                        # If sign detected normally, it flips!
                            ...
                            if CheckSign(img):
                                if IntentionalCrash(sct):
                                    time.sleep(0.9)
                                    GrindRight()
                            elif not CheckSign(img):
                                if IntentionalCrash(sct):
                                    time.sleep(0.9)
                                    GrindLeft()

                        elif TricksterCheck == True:
                            Trickster()
